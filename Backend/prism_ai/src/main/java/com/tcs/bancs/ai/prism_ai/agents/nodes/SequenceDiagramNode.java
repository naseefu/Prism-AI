...
/**
 * Sequence Diagram Node — builds the sequence diagram (lifelines + arrows)
 * <b>programmatically</b> from pre-fetched trace spans. No LLM involvement.
 *
...
@Slf4j
public class SequenceDiagramNode {

    private final TraceCorrelationService traceCorrelationService;

    /**
     * Build the sequence diagram from state and return the typed DTO.
     *
     * @return map containing either a {@link SequenceDiagramResponseDTO} under "ai-response",
    ...
    public Map<String, Object> apply(OverAllState state) {
        ...
        if (ObjectUtils.isEmpty(traceId)) {
            return Map.of("ai-response",
                    "I need a trace ID to build the sequence diagram. Please provide a traceId, " +
                    "e.g., \"Show sequence diagram for traceId abc123\".");
        }
